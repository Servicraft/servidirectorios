...
public class StatsWeekGUI {
    ...
    private static final Map<UUID, Integer> currentMonth = new HashMap<>();
    private static final Map<UUID, Map<Integer, Integer>> weekMap = new HashMap<>();

    public static void open(Player player, int slotIndex, int year, int month) {
        currentSlot.put(player.getUniqueId(), slotIndex);
        currentYear.put(player.getUniqueId(), year);
        currentMonth.put(player.getUniqueId(), month);
        boolean since = StatsYearGUI.isSincePurchase(player);
        Map<Integer, Integer> data = DatabaseManager.getClicksPerWeek(slotIndex, year, month, since ? DatabaseManager.getSlotPurchaseTime(slotIndex) : 0L);
        int size = Math.max(9, ((data.size() - 1) / 9 + 1) * 9);
        Inventory inv = Bukkit.createInventory(null, size, Message.STATS_WEEKS_TITLE.get().replace("{month}", String.valueOf(month)).replace("{year}", String.valueOf(year)));
        ...
        weekMap.put(player.getUniqueId(), slotMap);
    ...
    public static int getMonth(Player player) {
        return currentMonth.getOrDefault(player.getUniqueId(), 0);
    }
}
